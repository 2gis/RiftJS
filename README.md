...
В файл `App/Model.js` добавьте свойство `viewer` — текущий пользователь приложения. Должно получиться так:
```js
var Rift = require('riftjs');
...
Пересоберите проект (автоматическая пересборка скоро будет) и обновите страницу:
```
gulp --dev
```
...
Посмотрите сгенеренный сервером код. Там много непонятного, но это пока нормально.
